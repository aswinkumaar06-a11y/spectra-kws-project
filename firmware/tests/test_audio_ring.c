...
int main(void) {
    ...
    // ── Test 5: Circular Boundary Wrap-Around Stream ────────────────────
    printf("\n[5] Testing Continuous Streaming Across Circular Boundaries...\n");
    // Stream 20 consecutive hops (20 * 8000 = 160,000 samples, >3.3x buffer capacity)
    int hop_errors = 0;
    ...
    for (int h = 1; h <= 20; h++) {
        ...
        // Spot-check first and last sample of this window
        int16_t expected_first = (int16_t)(expected_start & 0xFFFF);
        int16_t expected_last = (int16_t)((expected_start + SPECTRA_WINDOW_SAMPLES - 1) & 0xFFFF);
        if (window[0] != expected_first || window[SPECTRA_WINDOW_SAMPLES - 1] != expected_last) {
            hop_errors++;
        }
    }
    CHECK(hop_errors == 0, "Streamed 20 hops (160,000 samples) with 0 boundary errors");
